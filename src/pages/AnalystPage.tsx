...
import { Briefcase, Plus, Trash2, Users, User, FolderOpen, FileText } from 'lucide-react';
...
export function AnalystPage() {
  ...
  const handleStartQueue = async () => {
    ...
    for (const file of importFiles) {
      ...
      const html = `<p>${parsed.content.replace(/\n\n/g, '</p><p>').replace(/\n/g, '<br>')}</p>`;
      items.push({
        title: parsed.title,
        contentHtml: html,
        contentPlain: parsed.content,
        date: parsed.date,
        clientId: client.id,
        ...
        <CardContent className="space-y-4">
          <div className="flex items-center gap-3">
            ...
            <Label htmlFor="analyst-mode" className="flex items-center gap-2 cursor-pointer">
              {analystMode
                ? <><Briefcase className="h-4 w-4 text-primary" /> Professional (multi-client)</>
          ...
          {!analystMode && (
            <p className="text-sm text-muted-foreground">
              You are in personal mode. Enable Professional mode to manage client journals and bulk-import dreams.
            </p>
          )}
        </CardContent>
      </Card>
        ...
        <>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="flex gap-2 items-end">
                ...
                <Button size="sm" className="gap-1.5" onClick={handleAddClient} disabled={!newName.trim()}>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Select multiple plain-text dream files for a client. The dream editor will open
                with each file pre-filled in sequence — review, edit, and save each one before
                moving to the next.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              {clients.length === 0 ? (
                ...
                <>
                  ...
                  <Button
                    ...
                    disabled={!importClientId || importFiles.length === 0}
                    className="gap-1.5"
                  >
                    <FolderOpen className="h-4 w-4" />
                    Start Import ({importFiles.length} file{importFiles.length !== 1 ? 's' : ''})
                  </Button>

                  <p className="text-xs text-muted-foreground">
                    Each file opens in the dream editor pre-filled with its content. Save each
                    entry to proceed to the next file in the queue.
                  </p>
                </>
              )}
            </CardContent>
